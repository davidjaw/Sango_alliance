...
def main():
    # Define the data augmentation pipeline using torchvision transforms
    transform = Compose([
    ...
    # transform = Compose([
    #     RandomApply([
    #         RandomRotation(10, fill=(0, 0, 0)),
    #         RandomAffine(degrees=0, scale=(0.9, 1.1), translate=(0.1, 0.1)),
    #     ], p=0.85),
    #     # Resize((96, 96)),
    #     ToTensor()
    # ])

    # Create the FontDataset instance
    dataset = CustomOCRDataset(5e3, transform=transform)

    # Create the PyTorch DataLoader
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=36, shuffle=True, num_workers=4)
    w = set()
    for images, labels in dataloader:
        ...
        for i in range(images_np.shape[0]):
            ...
            x = 0
            x_widths = wx[i]
            for x_idx in range(dataset.drawer.max_str_len):
                x_width = dataset.width_denormalizer(x_widths[x_idx])
                x += x_width
                img[:, x, :] = [0, 0, 1]
                w.add(x_width)
